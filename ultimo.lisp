...
(defun psr-variaveis-nao-atribuidas (psr)
	(let ((nao_atribuidas NIL))
		(dolist (var (psr-variables psr))
			(cond((not(gethash var (psr-varsHash psr)))
					(setf nao_atribuidas(append nao_atribuidas(list var)))
				 )
			)
		)
		nao_atribuidas
		  ...
		  (dotimes (i 3)
			(dotimes (j 3)
				...
				(if (and (>= newl 0) (< newl linhas) (>= newc 0) (< newc colunas)) 
						(setf lista (append lista (list (concatenate 'string (write-to-string newl) "_" (write-to-string newc)))))
				)
			)
		) 
		lista 
...
;esta funcao recebe um psr e o array correspondente, preenchendo a 1 as posicoes adjacentes (e inclusive) de casas 
;com valor 9, valor 4 nos cantos e valor 6 junto as paredes do tabuleiro. Preenche a 0 as de casas com valor 0.
...
;esta funcao recebe um psr e o array correspondente mudando o dominio para 0 das restantes casas a NIL adjacentes a uma casa que ja tem o maximo
;de casas adjacentes com valor 1. Muda tambem o dominio para 1 das casas a NIL quando a soma destas com as casas com valor 1 e igual ao numero de 
;casas a pintar.
