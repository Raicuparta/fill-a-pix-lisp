...
(defun procura-best (psr arr)
	(let ((testes-totais 0) (var NIL) (dominio NIL) (testes 0) (testes2 0) (resultado NIL) (teste 0)(lista-arcos NIL)(l_adj_na NIL) (backup-dominio NIL) (linhas 0) (colunas 0) (l 0) (c 0) (posicao NIL) (aux NIL) (inferencias NIL) (lista3 NIL) (consistente NIL) (lista2 NIL) (lista NIL))
		(cond ((psr-completo-p psr) (return-from procura-best (values psr testes-totais))))
		(setf numero 0)
		(setf var (escolhe-best psr arr))
 		(print var)
 		(print numero)
		...
		(dolist (value dominio)
			...
			(cond (consistente
					(psr-adiciona-atribuicao! psr var value)
					(setf l_adj_na (intersection (lista-adjacencias var linhas colunas) (psr-variaveis-nao-atribuidas psr)))	
					(cond ( (not l_adj_na)
							(setf aux (aref arr l c))
							(setf (aref arr l c) -1)))
							
					(dolist (var_na l_adj_na)	
					(setf lista-arcos (append lista-arcos (list (cons var_na var)))))
					
					(setf lista2 (multiple-value-bind (inferencias testes) (forward-checking-best psr var lista-arcos)(list inferencias testes)) )
					(setf inferencias (nth 0 lista2))
					(setf testes (nth 1 lista2))
					(setf testes-totais (+ testes-totais testes))
					...
					(cond ( (not (null inferencias))              ;(and (not (null inferencias)) (not (eq 0 (hash-table-count inferencias))))
						   ...
						   (setf lista3 (multiple-value-bind (resultado testes2) (procura-best psr arr)(list resultado testes2)) )
						   (setf resultado (nth 0 lista3))
						   (setf testes2 (nth 1 lista3))
						   ;(cond ((null testes2) (setf testes2 0)))
						   (setf testes-totais (+ testes-totais testes2))
						   (cond (resultado
									(return-from procura-best (values resultado testes-totais))))
							(loop for key being the hash-keys of backup-dominio do
											(psr-altera-dominio! psr key (gethash key backup-dominio)))))
					(psr-remove-atribuicao! psr var)
					(setf (aref arr l c) aux))))
		(values NIL testes-totais)
	)
)
...
(defun escolhe-best (psr arr)
	(let ((l_na NIL) (posicao NIL) (l 0) (c 0) (nr_var NIL))
		(setf l_na (psr-variaveis-todas psr))
		(dolist (var l_na)
			(setf posicao (posicao-divide var))
			(setf l (parse-integer (nth 0 posicao)))
			(setf c (parse-integer (nth 1 posicao)))
			(print nr_var)(prin1 var)
			(setf nr_var (aref arr l c))
			(cond ((not (eq nr_var -1))
					(cond ( (or (eq nr_var (+ 1 numero)) (eq nr_var (- 8 numero))) (return-from escolhe-best var))
					)
				  )
			)
		)
		(cond ((= numero 3) (return-from escolhe-best (first l_na))))
		(incf numero)
		(escolhe-best psr arr)
	)
)
...
;; 		(setf psr (preenche-resto psr arr))
;		(print (psr->fill-a-pix psr (array-dimension arr 0)(array-dimension arr 1)))
		(setf newpsr (procura-best psr arr))
		(setf newarr (psr->fill-a-pix newpsr (array-dimension arr 0)(array-dimension arr 1)))))

(defun forward-checking-best (psr var lista-arcos)
	(let ((dominio-v2 NIL) (testes-totais 0) (inferencias NIL) (testes 0) (revised NIL)(v1 NIL) (valores NIL)(v2 NIL) )
		(setf inferencias (make-hash-table :test 'equal))
		;(setf (gethash var inferencias) ())
		(dolist (el lista-arcos)
			(setf v2 (car el))
			...
			(cond (revised
					(setf dominio-v2 (gethash v2 inferencias))
					(cond ((zerop (length dominio-v2)) (return-from forward-checking-best (values NIL testes-totais))))))
		)
		(values inferencias testes-totais)
	)
)
